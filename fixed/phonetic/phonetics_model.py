...
def build_vocabularies(words_data):
    ...
    # Добавляем графемы (только те, что встречаются хотя бы 1 раз)
    for grapheme, count in grapheme_counter.items():
        if grapheme not in grapheme_to_idx:
            grapheme_to_idx[grapheme] = len(grapheme_to_idx)
    
    # Добавляем аллофоны
    for allophone, count in allophone_counter.items():
...
def train_phonetics_model():
    ...
    # Парсим XML
    words_data = parse_xml_for_phonetics('/home/artyom/itmo/syntes_rechi/fixed/gogol_utf8_cut.Result.xml')
    print(f"Загружено {len(words_data)} слов с фонетической разметкой")
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (graphemes, allophones, grapheme_mask, allophone_mask) in enumerate(train_loader):
        ...
        with torch.no_grad():
            for graphemes, allophones, grapheme_mask, allophone_mask in test_loader:
                graphemes = graphemes.to(device)
                allophones = allophones.to(device)
                grapheme_mask = grapheme_mask.to(device)
...
def create_phonetics_json(input_text, model, grapheme_to_idx, allophone_to_idx, output_file='phonetics_result.json'):
    """Создает JSON с фонетической транскрипцией"""
    
    # Разбиваем текст на слова
    words = re.findall(r'\b[\w-]+\b|[^\w\s]', input_text)
    
    # Предсказываем аллофоны
    allophone_predictions = predict_allophones(model, grapheme_to_idx, allophone_to_idx, words)
    ...
    for word, allophones in zip(words, allophone_predictions):
        result["words"].append({
            "content": word,
            "allophones": allophones
        })
    ...
    print(f"Фонетическая транскрипция сохранена в {output_file}")
    return result
...
if __name__ == "__main__":
    ...
    # Обучаем модель
    model, grapheme_to_idx, allophone_to_idx = train_phonetics_model()
    ...
    if model is not None:
        ...
        print("\n" + "=" * 60)
        print("Создание фонетической транскрипции для текста")
        print("=" * 60)
        
        test_text = "Привет мир, это тест фонетического транскриптора."
        result = create_phonetics_json(test_text, model, grapheme_to_idx, allophone_to_idx, 'phonetics_result.json')
        
        print("\nПример транскрипции:")
        for word_data in result["words"]:
            print(f"{word_data['content']}: {word_data['allophones']}")
        
        print(f"\nМодель успешно обучена и протестирована!")
        print(f"Файлы сохранены:")
        print(f"- best_phonetics_model.pth (модель)")
        print(f"- phonetics_result.json (пример транскрипции)")
    else:
